...
public class Day21a : PuzzleBaseClass
{
    ...
    public override string? OnSolve(ReadOnlySpan<char> inputFile)
    {
        ...
        foreach (var line in lines)
        {
            ...
            var allMovesRadiation = new List<string>();
            foreach (var move in shortestSequences)
                allMovesRadiation.AddRange(dirPad.GetMoveSequenceFor(move).Distinct());
            shortestSequence = allMovesRadiation.Min(x => x.Length);
            shortestSequences = allMovesRadiation.Where(x => x.Length == shortestSequence).Distinct().ToList();
            ...
            Console.WriteLine($"code: {line} sequence: {shortSequence} length: {sequenceLength}");
